...
void UKriegerUserSettings::SetToDefaults()
{
	Super::SetToDefaults();
...
EWindowMode::Type KriegerGetWindowModeType(bool bFullscreen)
{
	int32 FullScreenMode = KriegerGetBoundFullScreenModeCVar();
...
EWindowMode::Type UKriegerUserSettings::GetCurrentFullscreenMode() const
{
	...
	if ( GEngine && GEngine->GameViewport && GEngine->GameViewport->ViewportFrame )
	{
		...
		CurrentFullscreenMode = KriegerGetWindowModeType(bIsCurrentlyFullscreen);
	}
	return CurrentFullscreenMode;
}

void UKriegerUserSettings::SetLowQuality()
{
...
void UKriegerUserSettings::SetMidQuality()
{
	// Low-level options that may improve performance on low-end hardware.
	IConsoleVariable* CVarClearMethod = IConsoleManager::Get().FindConsoleVariable(TEXT("r.ClearSceneMethod"));
	CVarClearMethod->Set(0);
...
void UKriegerUserSettings::SetHighQuality()
{
	IConsoleVariable* CVarClearMethod = IConsoleManager::Get().FindConsoleVariable(TEXT("r.ClearSceneMethod"));
